...
{
    snd_pcm_t *playback_handle;         /* 재생 시간은 5초 */
    double total = DURATION, t;         /* 재생음의 주파수는 440Hz(A) */
    int freq = 440;                     /* 반복 횟수 */
    int i, frames, count = 1;           /* 오디오 디바이스의 이름 */
    char *snd_dev_out = "plughw:0,0";  /* 입력할 오디오를 위한 버퍼 */
    short buf[BUFSIZE];
