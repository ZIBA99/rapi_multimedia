...
int main(int argc, char **argv)                      /* 프레임 버퍼의 가로 × 세로의 크기 */
{
    ...
    pBmpData = (ubyte *)malloc(vinfo.xres * vinfo.yres * sizeof(ubyte) *
                              vinfo.bits_per_pixel/8);
    pData = (ubyte*)malloc(vinfo.xres * vinfo.yres * sizeof(ubyte) * color/8);

    /* 프레임 버퍼에 대한 메모리 맵을 수행한다. */
    pFbMap = (ubyte *)mmap(0, vinfo.xres * vinfo.yres * vinfo.bits_per_pixel/8,
    ...
    for(int y = 0, k, total_y; y < rows; y++) {
        ...
        for(int x = 0; x < cols; x++) {
            ...
            a = LIMIT_UBYTE(pData[k+x*color/8+3]);
